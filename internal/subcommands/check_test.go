...
func Test_performEmptyFolderAnalysis(t *testing.T) {
	...
	performCheck := true
	emptyDirName := "empty"
	if internal.MkdirForTesting(t, fnName, emptyDirName) != nil {
		return
	}
	defer func() {
	...
	dirtyDirName := "dirty"
	if internal.MkdirForTesting(t, fnName, dirtyDirName) != nil {
		return
	}
	defer func() {
	...
	internal.PopulateTopDirForTesting(t, fnName, dirtyDirName)
	type args struct {
		c *check
		s *files.Search
	}
