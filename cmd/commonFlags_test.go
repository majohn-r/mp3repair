	...
	if flag, ok := tfp.flags[name]; ok {
		if flag.valueKind == cmd.BoolType {
			if value, ok := flag.value.(bool); ok {
				...
				err = fmt.Errorf("code error: value for %q name is supposed to be bool, but it isn't", name)
			}
		} else {
	...
	if flag, ok := tfp.flags[name]; ok {
		if flag.valueKind == cmd.IntType {
			if value, ok := flag.value.(int); ok {
				...
				err = fmt.Errorf("code error: value for %q name is supposed to be int, but it isn't", name)
			}
		} else {
	...
	if flag, ok := tfp.flags[name]; ok {
		if flag.valueKind == cmd.StringType {
			if value, ok := flag.value.(string); ok {
				...
				err = fmt.Errorf("code error: value for %q name is supposed to be string, but it isn't", name)
			}
		} else {
...
func TestReadFlags(t *testing.T) {
	...
	tests := map[string]struct {
			...
			args: args{
				...
				defs: cmd.NewSectionFlags().WithSectionName("whatever").WithFlags(
					map[string]*cmd.FlagDetails{
						"misidentifiedBool":   cmd.NewFlagDetails().WithExpectedType(cmd.BoolType),
						"misidentifiedInt":    cmd.NewFlagDetails().WithExpectedType(cmd.IntType),
						"misidentifiedString": cmd.NewFlagDetails().WithExpectedType(cmd.StringType),
						"bool":                cmd.NewFlagDetails().WithExpectedType(cmd.BoolType),
						"int":                 cmd.NewFlagDetails().WithExpectedType(cmd.IntType),
						"string":              cmd.NewFlagDetails().WithExpectedType(cmd.StringType),
						"unexpected":          cmd.NewFlagDetails().WithExpectedType(cmd.UnspecifiedType),
					},
				),
			},
			want: map[string]*cmd.FlagValue{
		...
		"normal run": {
			args: args{
				producer: testFlagProducer{flags: map[string]testFlag{
					"specifiedBool":     {value: true, valueKind: cmd.BoolType, changed: true},
					"specifiedInt":      {value: 6, valueKind: cmd.IntType, changed: true},
					"specifiedString":   {value: "foo", valueKind: cmd.StringType, changed: true},
				...
				defs: cmd.NewSectionFlags().WithSectionName("whatever").WithFlags(
					map[string]*cmd.FlagDetails{
						"specifiedBool":     cmd.NewFlagDetails().WithExpectedType(cmd.BoolType),
						"specifiedInt":      cmd.NewFlagDetails().WithExpectedType(cmd.IntType),
						"specifiedString":   cmd.NewFlagDetails().WithExpectedType(cmd.StringType),
						"unspecifiedBool":   cmd.NewFlagDetails().WithExpectedType(cmd.BoolType),
						"unspecifiedInt":    cmd.NewFlagDetails().WithExpectedType(cmd.IntType),
						"unspecifiedString": cmd.NewFlagDetails().WithExpectedType(cmd.StringType),
					},
				),
			},
			want: map[string]*cmd.FlagValue{
		...
		"code error - missing details": {
			args: args{
				producer: nil,
				defs:     cmd.NewSectionFlags().WithFlags(map[string]*cmd.FlagDetails{"no deets": nil}),
			},
			want:  map[string]*cmd.FlagValue{},
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if len(got1) != len(tt.want1) {
				t.Errorf("ReadFlags() got1 = %d errors, want %d errors", len(got1), len(tt.want1))
			} else {
				for i, e := range got1 {
					if g := e.Error(); g != tt.want1[i] {
						t.Errorf("ReadFlags() got1[%d] = %q, want %q errors", i, e, tt.want1[i])
					}
				}
			}
		})
...
func TestFlagDetails_AddFlag(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"rejected string": {
			f: cmd.NewFlagDetails().WithUsage("a useful string").WithExpectedType(cmd.StringType).WithDefaultValue(""),
			args: args{
			...
			WantedRecording: output.WantedRecording{
				Error: "The configuration file \"defaults.yaml\" contains an invalid value for \"section\": rejecting default for flag.\n",
				Log:   "level='error' error='rejecting default for flag' section='section' msg='invalid content in configuration file'\n",
			},
		},
		"accepted string": {
			f: cmd.NewFlagDetails().WithUsage("a useful string").WithExpectedType(cmd.StringType).WithDefaultValue(""),
		...
		"accepted string with shorthand": {
			f: cmd.NewFlagDetails().WithAbbreviatedName("f").WithUsage("a useful string").WithExpectedType(cmd.StringType).WithDefaultValue(""),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{flags: map[string]*testFlagDatum{}},
				sectionName: "section",
		...
		"malformed string": {
			f: cmd.NewFlagDetails().WithUsage("a useful string").WithExpectedType(cmd.StringType).WithDefaultValue(true),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{},
				sectionName: "section",
			...
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: the type of flag \"flag\"'s value, 'true', is 'bool', but 'string' was expected.\n",
				Log:   "level='error' actual='bool' error='default value mistyped' expected='string' flag='flag' value='true' msg='internal error'\n",
			},
		},
		"rejected int": {
			f: cmd.NewFlagDetails().WithUsage("a useful int").WithExpectedType(cmd.IntType).WithDefaultValue(cmd_toolkit.NewIntBounds(0, 1, 2)),
			args: args{
				c:           testConfigSource{accept: false},
				flags:       &testFlagConsumer{},
				sectionName: "section",
		...
		"accepted int with shorthand": {
			f: cmd.NewFlagDetails().WithAbbreviatedName("i").WithUsage("a useful int").WithExpectedType(cmd.IntType).WithDefaultValue(cmd_toolkit.NewIntBounds(0, 1, 2)),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{flags: map[string]*testFlagDatum{}},
				sectionName: "section",
		...
		"malformed int": {
			f: cmd.NewFlagDetails().WithUsage("a useful int").WithExpectedType(cmd.IntType).WithDefaultValue(true),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{},
				sectionName: "section",
			...
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: the type of flag \"flag\"'s value, 'true', is 'bool', but '*cmd_toolkit.IntBounds' was expected.\n",
				Log:   "level='error' actual='bool' error='default value mistyped' expected='*cmd_toolkit.IntBounds' flag='flag' value='true' msg='internal error'\n",
			},
		},
		"rejected bool": {
			f: cmd.NewFlagDetails().WithUsage("a useful bool").WithExpectedType(cmd.BoolType).WithDefaultValue(false),
			args: args{
				c:           testConfigSource{accept: false},
				flags:       &testFlagConsumer{},
				sectionName: "section",
		...
		"accepted bool with shorthand": {
			f: cmd.NewFlagDetails().WithAbbreviatedName("b").WithUsage("a useful bool").WithExpectedType(cmd.BoolType).WithDefaultValue(false),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{flags: map[string]*testFlagDatum{}},
				sectionName: "section",
		...
		"malformed bool": {
			f: cmd.NewFlagDetails().WithUsage("a useful bool").WithExpectedType(cmd.BoolType).WithDefaultValue(cmd_toolkit.NewIntBounds(0, 1, 2)),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{},
				sectionName: "section",
			...
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: the type of flag \"flag\"'s value, '&{0 1 2}', is '*cmd_toolkit.IntBounds', but 'bool' was expected.\n",
				Log:   "level='error' actual='*cmd_toolkit.IntBounds' error='default value mistyped' expected='bool' flag='flag' value='&{0 1 2}' msg='internal error'\n",
			},
		},
		"malformed details": {
			f: cmd.NewFlagDetails().WithUsage("useless").WithExpectedType(cmd.UnspecifiedType).WithDefaultValue(true),
			args: args{
				c:           testConfigSource{accept: true},
				flags:       &testFlagConsumer{},
				sectionName: "section",
			...
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: unspecified flag type; section \"section\", flag \"flag\".\n",
				Log:   "level='error' default-type='bool' default='true' error='unspecified flag type' flag='flag' section='section' specified-type='0' msg='internal error'\n",
			},
		},
	}
	for name, tt := range tests {
...
func TestAddFlags(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"empty details with searches": {
			...
			wantNames: []string{"albumFilter", "artistFilter", "topDir", "trackFilter", "extensions"},
		},
		...
		"empty details with bad searches": {
			...
			replaceSearchFlags: cmd.NewSectionFlags().WithSectionName("common").WithFlags(
				map[string]*cmd.FlagDetails{
					"albumFilter":  cmd.NewFlagDetails().WithUsage("regular expression specifying which albums to select").WithExpectedType(cmd.StringType).WithDefaultValue(".*"),
					"artistFilter": cmd.NewFlagDetails().WithUsage("regular expression specifying which artists to select").WithExpectedType(cmd.StringType).WithDefaultValue(".*"),
					"topDir":       cmd.NewFlagDetails().WithUsage("top directory specifying where to find music files").WithExpectedType(cmd.BoolType).WithDefaultValue(filepath.Join("%HOMEPATH%", "Music")),
				},
			),
			doReplacement: true,
			wantNames:     []string{"albumFilter", "artistFilter"},
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: the type of flag \"topDir\"'s value, '%HOMEPATH%\\Music', is 'string', but 'bool' was expected.\n",
				Log:   "level='error' actual='string' error='default value mistyped' expected='bool' flag='topDir' value='%HOMEPATH%\\Music' msg='internal error'\n",
			},
		},
		"good details with good searches": {
			args: args{
				...
				defs: cmd.NewSectionFlags().WithSectionName("mySection").WithFlags(
					map[string]*cmd.FlagDetails{
						"myFlag": cmd.NewFlagDetails().WithExpectedType(cmd.BoolType).WithDefaultValue(false),
					},
				),
				includeSearches: true,
			},
			wantNames: []string{"myFlag", "albumFilter", "artistFilter", "topDir", "trackFilter", "extensions"},
		},
		...
		"bad details with good searches": {
			args: args{
				...
				defs: cmd.NewSectionFlags().WithSectionName("mySection").WithFlags(
					map[string]*cmd.FlagDetails{
						"myFlag":       cmd.NewFlagDetails().WithExpectedType(cmd.BoolType).WithDefaultValue(false),
						"myBadFlag":    cmd.NewFlagDetails().WithExpectedType(cmd.IntType).WithDefaultValue(false),
						"myBetterFlag": cmd.NewFlagDetails().WithExpectedType(cmd.BoolType).WithDefaultValue(true),
						"myWorseFlag":  cmd.NewFlagDetails().WithExpectedType(cmd.BoolType).WithDefaultValue("nope"),
					},
				),
				includeSearches: true,
			},
			wantNames: []string{"myFlag", "myBetterFlag", "albumFilter", "artistFilter", "topDir", "trackFilter", "extensions"},
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: the type of flag \"myBadFlag\"'s value, 'false', is 'bool', but '*cmd_toolkit.IntBounds' was expected.\n" +
					"An internal error occurred: the type of flag \"myWorseFlag\"'s value, 'nope', is 'string', but 'bool' was expected.\n",
				Log: "level='error' actual='bool' error='default value mistyped' expected='*cmd_toolkit.IntBounds' flag='myBadFlag' value='false' msg='internal error'\n" +
					"level='error' actual='string' error='default value mistyped' expected='bool' flag='myWorseFlag' value='nope' msg='internal error'\n",
			},
		},
		...
		"bad details with bad searches": {
			...
			WantedRecording: output.WantedRecording{
				Error: "An internal error occurred: there are no details for flag \"myAbsentDetails\".\n" +
					"An internal error occurred: the type of flag \"myBadFlag\"'s value, 'false', is 'bool', but '*cmd_toolkit.IntBounds' was expected.\n" +
					"An internal error occurred: the type of flag \"myWorseFlag\"'s value, 'nope', is 'string', but 'bool' was expected.\n" +
					"An internal error occurred: the type of flag \"topDir\"'s value, '%HOMEPATH%\\Music', is 'string', but 'bool' was expected.\n",
				Log: "level='error' error='no details present' flag='myAbsentDetails' section='mySection' msg='internal error'\n" +
					"level='error' actual='bool' error='default value mistyped' expected='*cmd_toolkit.IntBounds' flag='myBadFlag' value='false' msg='internal error'\n" +
					"level='error' actual='string' error='default value mistyped' expected='bool' flag='myWorseFlag' value='nope' msg='internal error'\n" +
					"level='error' actual='string' error='default value mistyped' expected='bool' flag='topDir' value='%HOMEPATH%\\Music' msg='internal error'\n",
			},
		},
	}
	for name, tt := range tests {
	...
	tests := map[string]struct {
		...
		"no results": {
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='no results to extract flag values from' msg='internal error'\n",
			},
		},
		"no such flag": {
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='flag not found' flag='myFlag' msg='internal error'\n",
			},
		},
		"flag has no data": {
			args: args{
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='no data associated with flag' flag='myFlag' msg='internal error'\n",
			},
		},
		"flag not bool": {
			args: args{
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='flag value not boolean' flag='myFlag' value='1' msg='internal error'\n",
			},
		},
		"good boolean": {
			args: args{
	...
	tests := map[string]struct {
		...
		"flag not int": {
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='flag value not int' flag='myFlag' value='false' msg='internal error'\n",
			},
		},
		"good int": {
			args: args{
	...
	tests := map[string]struct {
		...
		"flag not string": {
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='flag value not string' flag='myFlag' value='false' msg='internal error'\n",
			},
		},
		"good string": {
			args: args{
	...
	tests := map[string]struct {
		...
		"errors": {
			...
			WantedRecording: output.WantedRecording{
				...
				Log:   "level='error' error='generic flag error' msg='internal error'\n",
			},
		},
	}
	for name, tt := range tests {
